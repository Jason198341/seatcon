...
import { getScheduleById, deleteSchedule } from '../../services/scheduleService';
...
const ScheduleDetail = ({ id }) => {
  ...
  useEffect(() => {
    const loadSchedule = async () => {
      ...
      try {
        ...
        console.error('일정 상세 정보 로드 오류:', err);
  ...
  if (!schedule) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">{t('일정 정보를 찾을 수 없습니다.')}</p>
        <Link to="/schedules" className="mt-4 inline-flex items-center text-sm text-indigo-600 hover:text-indigo-500">
        ...
        <div className="flex items-center justify-between flex-wrap sm:flex-nowrap">
          <div>
            ...
            <p className="mt-1 text-sm text-indigo-100">
              {format(new Date(schedule.start_date), 'yyyy년 M월 d일', { locale: ko })}
              {schedule.start_date !== schedule.end_date && (
                <> ~ {format(new Date(schedule.end_date), 'yyyy년 M월 d일', { locale: ko })} ({getDurationText(schedule.start_date, schedule.end_date)})</>
              )}
      ...
      <div className="px-4 py-5 sm:px-6">
        <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
          <div className="sm:col-span-1">
            <dt className="text-sm font-medium text-gray-500">{t('유형')}</dt>
          ...
          <div className="sm:col-span-1">
            <dt className="text-sm font-medium text-gray-500">{t('위치')}</dt>
            <dd className="mt-1 text-sm text-gray-900">{schedule.location || t('위치 미정')}</dd>
          </div>
          
          <div className="sm:col-span-2">
            <dt className="text-sm font-medium text-gray-500">{t('설명')}</dt>
            <dd className="mt-1 text-sm text-gray-900">
              {schedule.description ? (
                <p className="whitespace-pre-line">{schedule.description}</p>
              ) : (
                <p className="text-gray-500">{t('설명 없음')}</p>
              )}
            </dd>
          </div>
          
          <div className="sm:col-span-2">
            <dt className="text-sm font-medium text-gray-500">{t('등록 정보')}</dt>
            <dd className="mt-1 text-sm text-gray-900">
              <time dateTime={schedule.created_at}>
                {format(new Date(schedule.created_at), 'yyyy-MM-dd HH:mm')}
              </time>
              {schedule.updated_at && schedule.updated_at !== schedule.created_at && (
                <span className="ml-2 text-gray-500">
                  ({t('최종 수정')}:{' '}
                  <time dateTime={schedule.updated_at}>
                    {format(new Date(schedule.updated_at), 'yyyy-MM-dd HH:mm')}
                  </time>
                  )
                </span>
              )}
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        {t('일정')} "{schedule.title}"{t('을(를) 정말로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')}
                      </p>
                    </div>
                  </div>
                </div>
